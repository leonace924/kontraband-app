...
export default class MultiSelect extends Component {
  ...
  _displaySelectedItems = optionalSelctedItems => {
    ...
    return actualSelectedItems.map(singleSelectedItem => {
      ...
      let lengthItem = !item[displayKey] ? singleSelectedItem.length : item[displayKey].length
      return (
        <View
          style={[
            styles.selectedItem,
            {
              width: lengthItem * 8 + 80,
              justifyContent: "center",
              height: 40,
              borderColor: tagBorderColor
            }
          ]}
          key={!item[displayKey] ? singleSelectedItem : item[uniqueKey]}
        >
          <Text
            style={[
              {
            ...
            numberOfLines={1}
          >
            {!item[displayKey] ? singleSelectedItem : item[displayKey]}
          </Text>
          <TouchableOpacity
            onPress={() => {
              this._removeItem(!item[displayKey] ? { name: singleSelectedItem, id: singleSelectedItem.split(" ").filter(word => word.length).join("-") } : item);
            }}
          >
            <MaterialCommunityIcons
              name="close-circle"
  ...
  _submitSelection = () => {
    const { selectedItems, onSelectedItemsChange, items } = this.props;
    const { searchTerm } = this.state;
    const renderItems = searchTerm ? this._filterItems(searchTerm) : items;
    if (this.state.searchTerm && !renderItems.length) {
      newSelectedItems = [];

      const newItemId = this.state.searchTerm
        .split(" ")
        .filter(word => word.length)
        .join("-");
      newSelectedItems = [...selectedItems, newItemId];
      onSelectedItemsChange(newSelectedItems);
      // this.props.addInputTag(this.state.searchTerm)
    }
  ...
  _getRowNew = item => (
    <TouchableOpacity
      ...
      style={{ paddingLeft: 20, paddingRight: 20, backgroundColor: 'green' }}
    >
      <View>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <Text
